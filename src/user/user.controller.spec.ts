...
//             expect(await userService.createUser(body)).toEqual(allUsers);
...
//             await expect(userService.createUser(body)).rejects.toThrow(new NotFoundException(`User with name ${body.name} already exist`));
//             expect(mockRepository.findOne).toHaveBeenCalledWith({ where: { name: body.name } }); // Check findOne instead of findByName
//           });
//     });
